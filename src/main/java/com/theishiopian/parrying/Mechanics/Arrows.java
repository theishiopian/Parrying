...
public abstract class Arrows
{
    ...
    public static void DoBurningArrow(AbstractArrow arrow, HitResult result)
    {
        if(arrow.isOnFire() && Config.flamingArrowGriefing.get() && result instanceof BlockHitResult hit)
        {
            ...
            if(arrow.getOwner() instanceof ServerPlayer && toBurn.is(Blocks.CAMPFIRE) && !toBurn.getValue(CampfireBlock.LIT))
            {
                ModTriggers.campfireLight.trigger((ServerPlayer) arrow.getOwner());
            }
        }
    }
}